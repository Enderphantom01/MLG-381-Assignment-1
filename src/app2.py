...
model_1_path = script_dir / 'model_1.pkl'
model_2_path = script_dir / 'model_2.pkl'
...
# Function to load models with better error handling
def load_models():
    models = {}
    try:
        logger.info(f"Attempting to load models from: {script_dir}")
        
        if model_1_path.exists():
            logger.info(f"Loading model 1 from: {model_1_path}")
            models['model_1'] = joblib.load(model_1_path)
            logger.info(f"Model 1 loaded successfully. Type: {type(models['model_1'])}")
        else:
            logger.error(f"Model 1 file not found at: {model_1_path}")
            
        if model_2_path.exists():
            logger.info(f"Loading model 2 from: {model_2_path}")
            models['model_2'] = joblib.load(model_2_path)
            logger.info(f"Model 2 loaded successfully. Type: {type(models['model_2'])}")
        else:
            logger.error(f"Model 2 file not found at: {model_2_path}")
            
    except Exception as e:
        logger.error(f"Error loading models: {str(e)}", exc_info=True)
        
    return models.get('model_1'), models.get('model_2')

# Function to load test data with better error handling
def load_test_data():
    try:
        if test_data_path.exists():
            logger.info(f"Loading test data from: {test_data_path}")
            data = pd.read_csv(test_data_path)
            logger.info(f"Test data loaded with {len(data)} rows")
            return data
        else:
            logger.warning(f"Test data file not found at: {test_data_path}. Creating sample data.")
            
    except Exception as e:
        logger.error(f"Error loading test data: {str(e)}", exc_info=True)
    
    # Create sample data if file doesn't exist or error occurs
    return pd.DataFrame({
...
# Load models and data with logging
logger.info("Starting application initialization")
model_1, model_2 = load_models()
test_data = load_test_data()

# Debugging output
logger.info(f"Model 1 type: {type(model_1) if model_1 is not None else 'None'}")
logger.info(f"Model 2 type: {type(model_2) if model_2 is not None else 'None'}")
...
def preprocess_data(input_data):
    try:
        ...
        # Standardize features for model 2 if it's a neural network
        if model_2 is not None:
            scaler = StandardScaler()
            X_scaled = scaler.fit_transform(X)
        else:
            X_scaled = X.values  # Fallback if no model 2
...
# Function to make predictions with better error handling
# Update the make_predictions function with better scaling
def make_predictions(input_data):
    try:
        X, X_scaled = preprocess_data(input_data)
        
        # Initialize default predictions
        rf_pred, xgb_pred = 2.0, 2.0  # Default average grade
        
        # Random Forest prediction
        if model_1 is not None:
            try:
                rf_pred = model_1.predict(X)[0]
                # If RF is predicting class probabilities
                if isinstance(rf_pred, np.ndarray) and len(rf_pred) > 1:
                    rf_pred = np.argmax(rf_pred)  # Take the class with highest probability
                logger.info(f"RF prediction raw: {rf_pred}")
            except Exception as e:
                logger.error(f"RF prediction error: {str(e)}", exc_info=True)
        
        # XGBoost prediction
        if model_2 is not None:
            try:
                xgb_pred = model_2.predict(X)[0]
                
                # Handle different prediction types
                if isinstance(xgb_pred, np.ndarray):
                    if len(xgb_pred) > 1:  # Multiclass probabilities
                        xgb_pred = np.argmax(xgb_pred)  # Take class with highest probability
                    else:  # Binary probability
                        xgb_pred = xgb_pred[0] * 4  # Scale 0-1 to 0-4
                elif xgb_pred < 1:  # Assume it's a probability if < 1
                    xgb_pred = xgb_pred * 4  # Scale 0-1 to 0-4
                
                logger.info(f"XGB prediction raw: {xgb_pred}")
            except Exception as e:
                logger.error(f"XGB prediction error: {str(e)}", exc_info=True)
        
        # Post-process predictions
        rf_pred = max(0.0, min(4.0, float(rf_pred)))
        xgb_pred = max(0.0, min(4.0, float(xgb_pred)))
        
        # Round to nearest 0.5 for grade points
        rf_pred = round(rf_pred * 2) / 2
        xgb_pred = round(xgb_pred * 2) / 2
        
        logger.info(f"Final predictions - RF: {rf_pred}, XGB: {xgb_pred}")
        return rf_pred, xgb_pred
        
    except Exception as e:
        logger.error(f"Prediction error: {str(e)}", exc_info=True)
        return 2.0, 2.0  # Return default values on error
...
def update_prediction(n_clicks, study_time, absences, tutoring, parental_support, engagement, attendance):
    ...
    # Make predictions
    rf_pred, xgb_pred = make_predictions(input_data)
    
    # Create prediction output
    rf_letter = get_letter_grade(rf_pred)
    xgb_letter = get_letter_grade(xgb_pred)
    ...
    prediction_output = html.Div([
        ...
        html.Div([
            ...
            html.H2(f"{xgb_pred:.1f} ({xgb_letter})", style={'color': colors['dark'], 'textAlign': 'center'})
    ...
    fig.add_trace(go.Indicator(
        ...
        title={'text': "XGBoost Model", 'font': {'size': 16}}
    ))
...
def update_model_comparison(n_clicks):
    ...
    X, _ = preprocess_data(test_data.drop('GradeClass', axis=1) if 'GradeClass' in test_data.columns else test_data)
    ...
    if model_2 is not None:
        if hasattr(model_2, 'predict_proba'):  # For classification models
            y_pred_xgb = model_2.predict_proba(X)[:, 1] * 4  # Scale to 0-4
    ...
    melted_df = pd.melt(comparison_df, id_vars=['Student'], 
                        value_vars=['Random Forest', 'XGBoost'] + (['Actual'] if y_actual is not None else []),
    ...
    for i, model in enumerate(['Random Forest', 'XGBoost'] + (['Actual'] if y_actual is not None else [])):
